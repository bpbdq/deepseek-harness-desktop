...
| 菜单 | 内容 |
|---|---|
| 文件 | **项目信息…**（`Ctrl+I`）、重新加载、强制重新加载、开发者工具、退出 |
...
- **Git 分支**，含未提交改动条数与相对上游的领先 / 落后（`main*  ↑2 ↓1`）
- 工作区路径
...
`DeepSeek Harness — main*`，一眼就能看出在哪个分支上工作。
...
官方 Web 界面里没有这个入口——它属于外壳层面，所以在**外壳自己的界面**里：

| 位置 | 入口 |
|---|---|
| 窗口菜单栏 | **更新 → 检查智能体运行时更新…**（`Ctrl+Shift+U`） |
| 窗口菜单栏 | 帮助 → 检查智能体运行时更新… |
| 托盘右键 | 检查运行时更新… |

检查结果会报告：已安装版本、所跟通道上的最新版本、**当前实际运行的是哪份运行时**（内置还是已下载）、安装位置、以及查询所用的源。“已是最新”只有在通道可见时才有意义。
...
- **shell 自更新已接线但未验证。** `electron-updater` 需要签名构建与真实发布源才能端到端测试。
